    ...
    <div class="container">
        ...
        <table>
            ...
            <tbody>
                ...
                $query = "SELECT * FROM `ouvrier` where idpepinier='$roi[pepiniere]'";
                $result = $con->query($query);

                // Affichage des données dans le tableau
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $quer = "SELECT * FROM `pépinière` where id='$row[idpepinier]'";
                        ...
                        echo "<td>{$ro['nom']}</td>";
                        echo "<td>";
                        echo "<a href='updateouvr.php?id={$row['id']}'>Modifier</a> | ";
                        echo "<a href='deletouvr.php?id={$row['id']}' onclick='return confirm(\"Voulez-vous vraiment supprimer ce ouvrier ?\")'>Supprimer</i></a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='7'>Aucun Ouvrier trouvé.</td></tr>";
                }
