...
<body>
 ...
 include('config.php');
     ...
     $idd = $_SESSION['user_id'];
                ...
                $query = "SELECT * FROM `plants` where idpepiniere='$roi[pepiniere]' ";
                $result = $con->query($query);

                // Affichage des données dans le tableau
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $quer = "SELECT * FROM `pépinière` where id='$row[idpepiniere]'";
                        ...
                        echo "<td>";
                        echo "<a href='updateplante.php?id={$row['id']}'>Modifier</a> | ";
                        echo "<a href='deletplante.php?id={$row['id']}' onclick='return confirm(\"Voulez-vous vraiment supprimer cette plante ?\")'>Supprimer</i></a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='7'>Aucun chef trouvé.</td></tr>";
                }
